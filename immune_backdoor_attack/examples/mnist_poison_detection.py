...
def main():
    ...
    model = Sequential()
    model.add(Conv2D(32, kernel_size=(7, 7), activation="relu", input_shape=x_train.shape[1:]))
    ...
    pyplot.suptitle('predict : 1' +' Label : 0')
    pyplot.imshow(x_test[is_poison_test][0], cmap=pyplot.get_cmap('gray'))
    pyplot.show()
    ...
    try:
        ...
        plt.title("Class " + str(n_class) + " cluster: Fail")
        plt.show()
        plt.imshow(sprites_by_class[n_class][1])
        plt.title("Class " + str(n_class) + " cluster: Success")
        plt.show()
    except ImportError:
...
def generate_backdoor(
    ...
    for i, (src, tgt) in enumerate(zip(sources, targets)):
        ...
        y_poison = np.append(y_poison, np.ones(num_poison) * 10, axis=0)
